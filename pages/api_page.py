...
class ApiPage:
    ...
    def search_book(self, search_phrase, customer_city):
        my_params = {"phrase": search_phrase, "customerCityId": customer_city}
        response = requests.get(
            self.url + "/api/v2/search/product",
    ...
    def add_book(self, book_id: int):
        ...
        response = requests.post(
            self.url + "/api/v1/cart/product", headers=self.my_headers, json=payload
        )
        return response
    ...
    def check_cart(self):
        response = requests.get(
            self.url + "/api/v1/cart/short", headers=self.my_headers
        )
    ...
    def delete_from_cart(self, book_id: int):
        response = requests.delete(
            self.url + f"/api/v1/cart/product/{book_id}", headers=self.my_headers
        )
        return response
